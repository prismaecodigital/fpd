            ...
            <div class="row">
              ...
              <div class="col-auto">
                <router-link v-if="($can(entry.bu.code + '-admin')) || entry.status == 0 || (entry.status === 1 && $can(entry.bu.code + '-' + parseInt(entry.status)-1))"
                    ...
                    :to="{ name: 'lrds.editData', params: { id: entry.id } }"
                    target="_blank"
                    >
                    Proses
                </router-link>
            ...
            <table class="table table-bordered" name="inputItem">
              ...
              <tbody>
                  <tr>
                    ...
                    <td>Rp. {{entry.total_real_amount_label ?? 0}}</td>
                    <td></td>
                    <td></td>
                  </tr>
                </tbody>
...
export default {
  ...
  computed: {
    ...mapGetters('LrdsSingle', ['entry', 'timelineData'])
  },
  watch: {
    '$route.params.id': {
      immediate: true,
  ...
  methods: {
    ...mapActions('LrdsSingle', ['fetchShowData', 'resetState']),
